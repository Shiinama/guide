...
import { IModal } from './utils/typings';

const STEP_NUMBER = (idx: number, length: number) => `Step ${idx} of ${length}`;
...
const Modal: React.FC<IModal> = ({
    ...
    const _okText =
        stepIndex !== steps.length - 1
            ? nextText || 'Next step'
            : okText || 'I know';
    const _prevText = prevText || 'Previous step';
    const _skipText = skipText || 'Skip';
    const _stepText = stepText || STEP_NUMBER;
    ...
    }, [anchorEl, mask, parentEl, scrollContainer, stepInfo]);
    ...
    return visible
        ? ReactDOM.createPortal(
              <div
                  ref={modalRef}
                  className={`absolute w-80 text-white z-[1000] bg-black shadow-xl p-4 text-left ${className}`}
                  style={modalStyle}
              >
                  {/* ARROW */}
                  {arrow && (
                      <span className="absolute w-2 h-2" style={arrowStyle} />
                  )}
                  {/* HOT SPOT */}
                  {hotspot && (
                      <div
                          className="absolute w-4 h-4 rounded-[50%] bg-[#4e83fd] animate-pulse"
                          style={hotspotStyle}
                      />
                  )}
                  {/* CLOSE button */}
                  ...
                  {/* MODAL TITLE */}
                  <div className="block font-bold h-6 mb-2">
                      {stepInfo.title}
                  </div>
                  {/* MODAL CONTENT */}
                  ...
                  {/* MODAL FOOTER */}
                  <div className="w-full max-h-6 flex items-center justify-between">
                      <span>{_stepText(stepIndex + 1, steps.length)}</span>
